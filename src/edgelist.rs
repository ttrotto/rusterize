/*
Build structured edge list from a polygon, where each
polygon is a list [x1 y1 x2 y2 ... xn yn] that is first
converted into a Nx2 matrix of nodes and then an edge list
...
pub fn build_edges(
    polygon: Vec<f64>,
    raster: &Raster
) -> Result<Vec<Edge>, ShapeError> {
    // build structured edge for each polygon
    let mut edges = Vec::new();
    // from vector to 2d array of xy pairs
    let nrows = (polygon.len() - 2) / 2; // drop last 2 entries because duplicates
    let node_array = Array::<f64, _>::from_shape_vec((nrows, 2), polygon)
